...
document.querySelectorAll('.clicableParaExpandir').forEach((clicable, index) => 
{

        // Set the title attribute to display the index on hover
        clicable.setAttribute('title', `Índice: ${index}`);

    clicable.addEventListener('click', () => {
        let inciso = document.querySelectorAll('.inciso')[index];

        // Mostrar los índices en la consola
        console.log(`Índice del "expandible" clicado: ${index}`);
        console.log(`Elemento clicado:`, clicable);
        console.log(`Índice del inciso asociado: ${index}`);
        console.log(`Elemento asociado (inciso):`, inciso);
